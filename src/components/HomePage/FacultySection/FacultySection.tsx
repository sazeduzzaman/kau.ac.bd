  ...
  const faculties = [
    {
      ...
      icon: <FaPiggyBank className=" text-" />,
      btnText: "Go To VABS",
    },
    {
      title: "Agriculture",
    ...
    },
    {
      ...
      icon: <FaSeedling className=" text-" />,
      btnText: "Go To HCS",
    },
  ];
  ...
  return (
    <section className="px-4 py-16 bg-gradient-to-b from-slate-50 to-white">
      <div className="mx-auto max-w-7xl">
        <h2 className="mb-12 text-4xl font-extrabold text-center text-site-secondary">
          Faculties of KAU
        ...
        <div className="grid grid-cols-1 gap-8 md:grid-cols-2 lg:grid-cols-3">
          {faculties.map((faculty, index) => (
            <div
              key={index}
              className="flex flex-col items-start h-full p-8 transition-transform transform bg-white border border-gray-100 shadow-lg faculty-cards rounded-xl hover:shadow-2xl hover:scale-105"
            >
              {/* Icon */}
              <div className="mb-4 text-site-accent faculty-icon" style={{fontSize:"140px !important"}}>
                {faculty.icon}
              </div>

              <h3 className="text-2xl font-bold text- mb-4 min-h-[3.5rem] uppercase" style={{marginTop: "-150px"}}>
                {faculty.title}
              </h3>
